...
export function showTimezoneMode(model, dispatch) {
    return div(empty(), singleton(react.createElement("div", {}, checkbox(empty(), ofArray([input_1(singleton(new Common_GenericOption(1, ofArray([new HTMLAttr(39, !equals(model.SearchMode, new SearchMode(0))), new HTMLAttr(22, equals(model.TimeZone, new TimeZone(1))), new DOMAttr(9, (evt) => {
...
export function showCustomInterval(model, dispatch) {
    ...
    const disabled = equals(model.SearchMode, new SearchMode(1));
    return div_1(singleton(new Option_1(4)), ofArray([showButton("Custom interval", getColor(new SearchInterval(3)), disabled, new Msg(8), dispatch), showSpace(), showTimezoneMode(model, dispatch)]));
}

export function showIntervalRange(model, dispatch) {
    const timeMode = (model.TimeZone.tag === 1) ? "local" : "UTC";
    return div_1(singleton(new Option_1(4)), ofArray([showIntervalTime(model.FromTime, toText(printf("from %s time"))(timeMode), ModelModule_hasValidFromTime, (arg0) => (new Msg(10, arg0)), model, dispatch), showSpace(), showIntervalTime(model.ToTime, toText(printf("to %s time"))(timeMode), ModelModule_hasValidToTime, (arg0_1) => (new Msg(11, arg0_1)), model, dispatch)]));
}

export function showSettings(model, dispatch) {
    return column(empty(), ofArray([showSearchModes(model.SearchMode, dispatch), showIdInput(model, dispatch), showSearchIntervals(["1 hour", "4 hours", "24 hours"], empty(), model, dispatch), showCustomInterval(model, dispatch), showIntervalRange(model, dispatch)]));
}
...
export function showResultRow(eventRow, i, model, dispatch) {
    ...
    const modifiers = singleton(new Modifier_IModifier(3, new Screen(0), new TextSize_Option(6)));
    const columns = ofArray([react.createElement("td", {}, Text_span(singleton(new Common_GenericOption(2, modifiers)), singleton(eventRow.CollectionName))), react.createElement("td", {}, Text_span(singleton(new Common_GenericOption(2, cons(new Modifier_IModifier(2, new TextWeight_Option(3)), modifiers))), singleton(eventRow.EventId))), react.createElement("td", {}, Text_span(singleton(new Common_GenericOption(2, modifiers)), singleton(eventRow.Description))), react.createElement("td", {}, Text_span(singleton(new Common_GenericOption(2, modifiers)), singleton(eventRow.Date))), react.createElement("td", {}, Text_span(singleton(new Common_GenericOption(2, modifiers)), singleton(eventRow.Time)))]);
...
export function showResults(model, dispatch) {
    return column(empty(), singleton(div_1(empty(), singleton(react.createElement("div", {}, table(ofArray([new TableOption(3), new TableOption(4)]), singleton(react.createElement("tbody", {}, ...showResultRows(model, dispatch)))))))));
}

export function showContent(model, dispatch) {
    const modifiers = ofArray([new Modifier_IModifier(5, new Screen(0), new TextAlignment_Option(2)), new Modifier_IModifier(0, new Color_IColor(2)), new Modifier_IModifier(1, new Color_IColor(9)), new Modifier_IModifier(3, new Screen(0), new TextSize_Option(6))]);
    return column(empty(), singleton(div_1(empty(), singleton(react.createElement("div", {}, content(singleton(new Option_3(3, modifiers)), singleton(react.createElement("p", {}, Model__get_ContentAsString(model)))))))));
}

export function view(model, dispatch) {
    return columns_1(empty(), ofArray([column(singleton(new Option_4(0, new Screen(0), new ISize(0))), singleton(showControls(model, dispatch))), column(singleton(new Option_4(0, new Screen(0), new ISize(0))), singleton(showResults(model, dispatch))), column(singleton(new Option_4(0, new Screen(0), new ISize(2))), singleton(showContent(model, dispatch)))]));
}
